...
pub struct Search<T> {
    ...
    /// Check whether we want to compute a binary tree-child network
    binary: bool,
}
...
impl<T: Clone> Search<T> {
    ...
    pub fn branch(&mut self) -> bool {
        ...
        if let Some((cherry, first_leaf, cut_count)) = self.next_branch_not_skipped() {
            ...
            let (u, v) = if first_leaf { (u, v) } else { (v, u) };

            // Cutting u is allowed only if v has not been cut yet and necessary only if u has not
            // been cut in all trees of the current cherry yet.  If we're looking for a binary
            // tree-child network, then we can add only only non-trivial cherry (u, v) to the
            // sequence, for every leaf u.
            if self.state.leaf(v).num_occurrences() == self.state.num_trees()
                && (!self.use_redundant_branch_opt || cut_count < cherry.num_occurrences())
                && (!self.binary || self.state.leaf(u).times_pruned() == 0)
            {
    ...
    fn next_branch_not_skipped(&mut self) -> Option<(cherry::Cherry, bool, usize)> {
        ...
        Some((
            self.remove_cherry(cherry::Ref::NonTrivial(cherry)),
            first_leaf,
            cut_count,
        ))
    }
    ...
    fn next_branch(&mut self) -> (usize, bool, usize) {
        let (cherry, first_leaf, num_branches_left) = {
            ...
            (
                branch_point.cherry,
                branch_point.first_leaf,
                branch_point.num_branches_left,
            )
        };
    ...
    fn rewind_to_snapshot(&mut self, snapshot: Snapshot) {
        for op in self.history.rewind(snapshot) {
            match op {
                ...
                Op::Cut(cherry, first_leaf, cut_count) => {
                    self.restore_cut_count(cherry, first_leaf, cut_count)
                }
    ...
    fn remove_cherry(&mut self, cherry_ref: cherry::Ref) -> cherry::Cherry {
        ...
        self.history
            .record_op(Op::RemoveCherry(cherry_ref, cherry.clone()));
        cherry
    }
    ...
    fn cut(&mut self, cherry_ref: cherry::Ref, first_leaf: bool) -> usize {
        ...
        self.history
            .record_op(Op::Cut(cherry_ref, first_leaf, cut_count));
        cut_count
    }
    ...
    fn restore_cut_count(&mut self, cherry_ref: cherry::Ref, first_leaf: bool, cut_count: usize) {
        self.state
            .restore_cut_count(cherry_ref, first_leaf, cut_count);
    }
...
mod tests {
    ...
    fn do_undo_increase_weight() {
        ...
        assert_eq!(
            history::tests::ops(&search.history),
            &vec![history::Op::IncreaseWeight]
        );
    ...
    fn do_undo_push_tree_child_pair() {
        ...
        assert_eq!(
            history::tests::ops(&search.history),
            &vec![history::Op::PushTreeChildPair]
        );
        assert_eq!(
            state::tests::tc_seq(&search.state),
            &vec![Pair::NonTrivial(u1, v1)]
        );
    ...
    fn do_undo_prune_leaf_suppress_node() {
        ...
        assert_eq!(
            newick::format_tree(state::tests::tree(&search.state, 0)),
            Some(newick0_0.clone())
        );
        assert_eq!(
            newick::format_tree(state::tests::tree(&search.state, 1)),
            Some(newick1_0.clone())
        );
        assert_eq!(
            newick::format_tree(state::tests::tree(&search.state, 2)),
            Some(newick2.clone())
        );
    ...
    fn do_undo_remove_cherry() {
        ...
        assert_eq!(
            state::tests::leaf(&search.state, 0)
                .cherries()
                .collect::<Vec<&cherry::Ref>>(),
            vec![&cherry::Ref::NonTrivial(0)]
        );
        assert_eq!(
            state::tests::leaf(&search.state, 1)
                .cherries()
                .collect::<Vec<&cherry::Ref>>(),
            vec![&cherry::Ref::NonTrivial(0), &cherry::Ref::NonTrivial(3)]
        );
        assert_eq!(
            state::tests::leaf(&search.state, 2)
                .cherries()
                .collect::<Vec<&cherry::Ref>>(),
            vec![&cherry::Ref::NonTrivial(3)]
        );
        assert_eq!(
            state::tests::leaf(&search.state, 3)
                .cherries()
                .collect::<Vec<&cherry::Ref>>(),
            vec![&cherry::Ref::NonTrivial(1)]
        );
        assert_eq!(
            state::tests::leaf(&search.state, 4)
                .cherries()
                .collect::<Vec<&cherry::Ref>>(),
            vec![&cherry::Ref::NonTrivial(1), &cherry::Ref::NonTrivial(2)]
        );
        assert_eq!(
            state::tests::leaf(&search.state, 5)
                .cherries()
                .collect::<Vec<&cherry::Ref>>(),
            vec![&cherry::Ref::Trivial(0)]
        );
        assert_eq!(
            state::tests::leaf(&search.state, 6)
                .cherries()
                .collect::<Vec<&cherry::Ref>>(),
            vec![&cherry::Ref::Trivial(0)]
        );
        assert_eq!(
            state::tests::leaf(&search.state, 7)
                .cherries()
                .collect::<Vec<&cherry::Ref>>(),
            vec![&cherry::Ref::NonTrivial(2)]
        );
        assert_eq!(state::tests::num_trivial_cherries(&search.state), 1);
        assert_eq!(
            state::tests::trivial_cherry(&search.state, 0).leaves(),
            (Leaf::new(5), Leaf::new(6))
        );
        assert_eq!(
            state::tests::trivial_cherry(&search.state, 0).indices(),
            (0, 0)
        );
        assert_eq!(
            state::tests::trivial_cherry(&search.state, 0)
                .trees()
                .collect::<Vec<&usize>>(),
            vec![&0, &1, &2]
        );
        ...
        assert_eq!(
            state::tests::non_trivial_cherry(&search.state, 0).leaves(),
            (Leaf::new(0), Leaf::new(1))
        );
        assert_eq!(
            state::tests::non_trivial_cherry(&search.state, 0).indices(),
            (0, 0)
        );
        assert_eq!(
            state::tests::non_trivial_cherry(&search.state, 0)
                .trees()
                .collect::<Vec<&usize>>(),
            vec![&0]
        );
        assert_eq!(
            state::tests::non_trivial_cherry(&search.state, 1).leaves(),
            (Leaf::new(3), Leaf::new(4))
        );
        assert_eq!(
            state::tests::non_trivial_cherry(&search.state, 1).indices(),
            (0, 0)
        );
        assert_eq!(
            state::tests::non_trivial_cherry(&search.state, 1)
                .trees()
                .collect::<Vec<&usize>>(),
            vec![&0]
        );
        assert_eq!(
            state::tests::non_trivial_cherry(&search.state, 2).leaves(),
            (Leaf::new(4), Leaf::new(7))
        );
        assert_eq!(
            state::tests::non_trivial_cherry(&search.state, 2).indices(),
            (1, 0)
        );
        assert_eq!(
            state::tests::non_trivial_cherry(&search.state, 2)
                .trees()
                .collect::<Vec<&usize>>(),
            vec![&1]
        );
        assert_eq!(
            state::tests::non_trivial_cherry(&search.state, 3).leaves(),
            (Leaf::new(1), Leaf::new(2))
        );
        assert_eq!(
            state::tests::non_trivial_cherry(&search.state, 3).indices(),
            (1, 0)
        );
        assert_eq!(
            state::tests::non_trivial_cherry(&search.state, 3)
                .trees()
                .collect::<Vec<&usize>>(),
            vec![&1]
        );
        ...
        assert_eq!(
            state::tests::leaf(&search.state, 1)
                .cherries()
                .collect::<Vec<&cherry::Ref>>(),
            vec![&cherry::Ref::NonTrivial(0), &cherry::Ref::NonTrivial(1)]
        );
        assert_eq!(
            state::tests::leaf(&search.state, 2)
                .cherries()
    ...
    fn do_undo_push_pop_trivial_cherry() {
        ...
        assert_eq!(
            state::tests::leaf(&search.state, 3)
                .cherries()
                .collect::<Vec<&cherry::Ref>>(),
            vec![&cherry::Ref::NonTrivial(1), &cherry::Ref::Trivial(1)]
        );
        assert_eq!(
            state::tests::leaf(&search.state, 4)
                .cherries()
        ...
        assert_eq!(
            state::tests::leaf(&search.state, 5)
                .cherries()
                .collect::<Vec<&cherry::Ref>>(),
            vec![&cherry::Ref::Trivial(0), &cherry::Ref::Trivial(1)]
        );
        assert_eq!(
            state::tests::leaf(&search.state, 6)
                .cherries()
        ...
        assert_eq!(
            state::tests::trivial_cherry(&search.state, 1).leaves(),
            (Leaf::new(3), Leaf::new(5))
        );
        assert_eq!(
            state::tests::trivial_cherry(&search.state, 1).indices(),
            (1, 1)
        );
        assert_eq!(
            state::tests::trivial_cherry(&search.state, 1)
                .trees()
                .collect::<Vec<&usize>>(),
            vec![&0]
        );
    ...
    fn do_undo_record_cherry() {
        ...
        assert_eq!(
            state::tests::leaf(&search.state, 1)
                .cherries()
                .collect::<Vec<&cherry::Ref>>(),
            vec![&cherry::Ref::NonTrivial(0), &cherry::Ref::NonTrivial(2)]
        );
        assert_eq!(
            state::tests::leaf(&search.state, 2)
                .cherries()
    ...
    fn resolve_trivial_cherries() {
        ...
        assert_eq!(
            state::tests::leaf(&search.state, 6)
                .cherries()
                .collect::<Vec<&cherry::Ref>>(),
            vec![&cherry::Ref::NonTrivial(4), &cherry::Ref::NonTrivial(5)]
        );
        assert_eq!(
            state::tests::leaf(&search.state, 7)
                .cherries()
                .collect::<Vec<&cherry::Ref>>(),
            vec![&cherry::Ref::NonTrivial(2), &cherry::Ref::NonTrivial(4)]
        );
        assert_eq!(state::tests::num_trivial_cherries(&search.state), 0);
        ...
        assert_eq!(
            state::tests::non_trivial_cherry(&search.state, 4).leaves(),
            (Leaf::new(6), Leaf::new(7))
        );
        assert_eq!(
            state::tests::non_trivial_cherry(&search.state, 4).indices(),
            (0, 1)
        );
        assert_eq!(
            state::tests::non_trivial_cherry(&search.state, 4)
                .trees()
                .collect::<Vec<&usize>>(),
            vec![&0]
        );
        assert_eq!(
            state::tests::non_trivial_cherry(&search.state, 5).leaves(),
            (Leaf::new(1), Leaf::new(6))
        );
        assert_eq!(
            state::tests::non_trivial_cherry(&search.state, 5).indices(),
            (2, 1)
        );
        assert_eq!(
            state::tests::non_trivial_cherry(&search.state, 5)
                .trees()
                .collect::<Vec<&usize>>(),
            vec![&2]
        );
