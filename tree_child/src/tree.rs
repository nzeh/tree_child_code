...
/// The internal node representation used by a tree
struct NodeRep {
    ...
    /// The data stored with this node
    node_data: NodeData,
}
...
pub enum NodeData {
    ...
    /// A leaf stores nothing but a label
    Leaf(usize),

    /// An internal node has a first child
    Internal(Node),
}

/// A representation of an operation performed on a tree in order to be able
/// to undo this operation.  Correctness depends that operations are undone
/// in the reverse order they are performed.
pub struct TreeOp;
...
impl NodeRep {
    ...
    /// Convenience constructor to create an isolated node with the given node data
    fn new(node_data: NodeData) -> NodeRep {
        NodeRep {
            parent: None,
            left:   None,
            right:  None,
